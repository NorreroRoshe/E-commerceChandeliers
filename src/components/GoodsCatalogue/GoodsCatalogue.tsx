...
const list: SortItem[] = [
  ...
  { name: 'По популярности (DESC)', sortProperty: { direction: 'desc', name: 'raiting' } },
  { name: 'По популярности (ASC)', sortProperty: { direction: 'asc', name: 'raiting' } },
  { name: 'По цене (DESC)', sortProperty: { direction: 'desc', name: 'price' } },
  { name: 'По цене (ASC)', sortProperty: { direction: 'asc', name: 'price' } },
  { name: 'По алфавиту (ASC)', sortProperty: { direction: 'desc', name: 'name' } },
  { name: 'По алфавиту (ASC)', sortProperty: { direction: 'asc', name: 'name' } },
];
...
// Object.entries(arr) => [['category', 'string']]

function makeParams(params: ProductParams) {
...
export const GoodsCatalogue: React.FC = () => {
  ...
  const onChangeSort = (number: Sort) => {
    dispatch(setSort(number));
  };
    ...
    <section className={s.section_catalogue}>
      <div className={`${s.catalogue__container} ${s.container}`}>
        ...
        <div className={s.catalogue__product}>
          <Sorts value={sort} onChangeSort={onChangeSort} list={list} title={'Сортировано по:'} />
          <h3 className={s.allproduct_heading}>Показано {122} товара</h3>
          {status === 'error' ? (
