...
import { SortItem } from './GoodsCatalogue/GoodsCatalogue';
...
export const Sorts: React.FC<SortProps> = React.memo(({ list, title, value, onChangeSort }) => {
  const [open, setOpen] = React.useState<boolean>(false);
  const sortRef = React.useRef<HTMLDivElement>(null);
  useOutsideAlerter(sortRef, () => setOpen(false));

  const onClickListItem = (obj: SortItem) => {
    onChangeSort(obj); //Делаем так, когда хотим чтобы чтонибудь выбралось
